...
{
    public class BnahabraGreen : ModNPC
    {
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            if (NPC.life <= 0)
            {
                ...
                {
                    Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, Mod.Find<ModGore>("BnahabraGreen1").Type, 1f);
                    Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, Mod.Find<ModGore>("BnahabraGreen2").Type, 1f);
                }
            }
        }
        ...
        {
            return (spawnInfo.Player.ZoneJungle || (spawnInfo.Player.ZoneCorrupt || spawnInfo.Player.ZoneCrimson)) && !spawnInfo.Water &&
        ...
        {
            npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<InsectShell>(), 50));
            npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<MonsterFluid>(), 50));
            if (Main.hardMode)
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<InsectCarapace>(), 50));
            }
        }
    }
}
